...
const int solenoidPin = 34;   
Adafruit_Fingerprint finger = Adafruit_Fingerprint(&Serial1); // Mega Serial1 (TX1=18, RX1=19)
...
void setup() {
  ...
  digitalWrite(solenoidPin, LOW); // locked initially
...
void controlDustbinLid() {
  ...
  if (fillDist < 0) fillDist = dustbinHeight; 

  if (fillDist >= 5) dustbinLevel = 0;       
  else if (fillDist >= 4) dustbinLevel = 1;  
  else if (fillDist >= 3) dustbinLevel = 2;  
  else if (fillDist >= 2) dustbinLevel = 4;  
  else if (fillDist >= 1) dustbinLevel = 3;  
  else dustbinLevel <= 1;                     
