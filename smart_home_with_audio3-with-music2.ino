...
int currentSongIndex = -1;  // Tracks current song index
...
void processVoiceCommand(String command) {
  ...
  // --- Music Controls ---
  if (rest == "play music") {
    if (!musicPlaying && currentSongIndex >= 0) {
      myDFPlayer.start();
      musicPlaying = true;
      lcd.clear(); lcd.print("Music Resumed");
    } else if (currentSongIndex == -1 && songCount > 0) {
      currentSongIndex = 0;
      myDFPlayer.play(songs[currentSongIndex].track);
      musicPlaying = true;
      lcd.clear(); lcd.print("Playing: "); lcd.print(songs[currentSongIndex].display);
    }
    return;
  }

  if (rest == "stop music" || rest == "pause music") {
    myDFPlayer.pause();
    musicPlaying = false;
    lcd.clear(); lcd.print("Music Paused");
    return;
  }

  if (rest == "next" || rest == "next song") {
    currentSongIndex = (currentSongIndex + 1) % songCount;
    myDFPlayer.play(songs[currentSongIndex].track);
    musicPlaying = true;
    lcd.clear(); lcd.print("Next: "); lcd.print(songs[currentSongIndex].display);
    return;
  }

  if (rest == "previous" || rest == "previous song") {
    currentSongIndex = (currentSongIndex - 1 + songCount) % songCount;
    myDFPlayer.play(songs[currentSongIndex].track);
    musicPlaying = true;
    lcd.clear(); lcd.print("Prev: "); lcd.print(songs[currentSongIndex].display);
    return;
  }
...
void updateLCD() {
  ...
  lcd.clear();
  int countOn = tableState + bedroomState + rebootState + garageState +
                securityState + alarmState + fanState + fenceState;
  lcd.setCursor(0,0);
