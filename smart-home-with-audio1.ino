...
void setDevice(bool on, int relayPin, bool &stateRef, int audioNumOn, int audioNumOff) {
  ...
  if (digitalRead(DFPLAYER_BUSY_PIN) == LOW) return; // wait if busy
...
void setAll(bool on) {
  ...
  setDevice(on, relayReboot, rebootState, 5, 5);    // Reboot ON/OFF (same audio)
...
void processBluetoothCommand(char cmd) {
  switch(cmd) {
    ...
    case 'E': garageDoorServo.write(90), garageDoorOpen=true, myDFPlayer.play(16); break; // Garage Open: 0016.mp3
    case 'F': garageDoorServo.write(0), garageDoorOpen=false, myDFPlayer.play(17); break; // Garage Close: 0017.mp3
  }
}
...
void processVoiceCommand(String command) {
  ...
  if (rest == "all on") { setAll(true); myDFPlayer.play(18); return; } // All ON: 0018.mp3
  if (rest == "all off") { setAll(false); myDFPlayer.play(19); return; } // All OFF: 0019.mp3

  bool turnOn = rest.endsWith(" on");
  bool turnOff = rest.endsWith(" off");

  if (rest == "open garage") { garageDoorServo.write(90); garageDoorOpen=true; myDFPlayer.play(16); return; }
  if (rest == "close garage") { garageDoorServo.write(0); garageDoorOpen=false; myDFPlayer.play(17); return; }
