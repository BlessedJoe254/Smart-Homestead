...
void setAll(bool on) {
  ...
  setDevice(on, relayKitchen,  kitchenState);
...
void setup() {
  ...
  digitalWrite(relayKitchen, HIGH);
...
void processBluetoothCommand(char cmd) {
  ...
  else if (cmd == '4') setDevice(true,  relayKitchen,  kitchenState);
  else if (cmd == '5') setDevice(false, relayKitchen,  kitchenState);
...
void processVoiceCommand(String command) {
  ...
  else if (device.indexOf("kitchen") != -1)
    setDevice(turnOn, relayKitchen, kitchenState);
...
void updateLCD() {
  ...
  if (countOn >= 2) {
    ...
    if (kitchenState) { lcd.print("Kitchen ON");   return; }
